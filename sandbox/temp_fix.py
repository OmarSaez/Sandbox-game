...
content = re.sub(r'func _update_pipe_visuals\(p, pass_index = -1\):.*?(?=func _update_pipe_x2_visuals\(p, pass_index = -1\):)', pipe1_code + '\n\n', content, flags=re.DOTALL)
content = re.sub(r'func _update_pipe_x2_visuals\(p, pass_index = -1\):.*?(?=func _is_orientation_allowed)', pipe2_code + '\n\n', content, flags=re.DOTALL)
